...
class GraphNetworkAVEC(torch.nn.Module):
    def __init__(self, num_features, num_relations, max_seq_len, hidden_size=64, dropout=0.5,
                 no_cuda=False):
        """
        The Speaker-level context encoder in the form of a 2 layer GCN.
        """
        super(GraphNetworkAVEC, self).__init__()
        ...
        self.linear = nn.Linear(num_features + hidden_size, hidden_size)
        self.dropout = nn.Dropout(dropout)
        self.smax_fc = nn.Linear(hidden_size, 1)
        self.no_cuda = no_cuda
...
class ResidualGCN(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len,
        ...
        ### ResidualNet
        self.vggres = VDCNN(depth=29, input_embed_size=600, output_embed_size=128, k=2)
        ...
        if self.avec:
            self.graph_net = GraphNetworkAVEC(2 * D_e, n_relations, max_seq_len, graph_hidden_size, dropout, self.no_cuda)
        else:
    ...
    def forward(self, U, qmask, umask, seq_lengths):
        ...
        if self.base_model == "DialogRNN":
            ...
            if self.avec:
                ...
                rnn_input = torch.cat((U, vggres_fea), 2)
                emotions_f, alpha_f = self.dialog_rnn_f(rnn_input, qmask)  # seq_len, batch, D_e
                rev_U = self._reverse_seq(rnn_input, umask)
                rev_qmask = self._reverse_seq(qmask, umask)
                emotions_b, alpha_b = self.dialog_rnn_r(rev_U, rev_qmask)
                emotions_b = self._reverse_seq(emotions_b, umask)
                emotions = torch.cat([emotions_f, emotions_b], dim=-1)
            ...
            lstm_input = torch.cat((U, vggres_fea), 2)
            emotions, hidden = self.lstm(lstm_input)
            #emotions, hidden = self.lstm(U)

        elif self.base_model == 'GRU':
            gru_input = torch.cat((U, vggres_fea), 2)
            emotions, hidden = self.gru(gru_input)
...
class ResidualGCNEC(nn.Module):

    def __init__(self, base_model, D_emb, D_m, D_r, D_g, D_p, D_e, D_h, D_a, word_embeddings, graph_hidden_size, n_speakers, max_seq_len,
                 window_past, window_future,
                 n_classes=7, listener_state=False, context_attention='simple', dropout_rec=0.5, dropout=0.5,
                 nodal_attention=True, avec=False, no_cuda=False):

        super(ResidualGCNEC, self).__init__()
        ...
        # The base model is the sequential context encoder.
        if self.base_model == 'DialogRNN':
            self.dialog_rnn_f = DialogueRNN(D_r, D_g, D_p, D_e, listener_state, context_attention, D_a, dropout_rec)
            self.dialog_rnn_r = DialogueRNN(D_r, D_g, D_p, D_e, listener_state, context_attention, D_a, dropout_rec)

        elif self.base_model == 'LSTM':
            self.lstm = nn.LSTM(input_size=D_r, hidden_size=D_e, num_layers=2, bidirectional=True, dropout=dropout)

        elif self.base_model == 'GRU':
            self.gru = nn.GRU(input_size=D_r, hidden_size=D_e, num_layers=2, bidirectional=True, dropout=dropout)

        elif self.base_model == 'None':
            self.base_linear = nn.Linear(D_r, 2 * D_e)
        ...
        ### ResidualNet
        self.vggres = VDCNN(depth=29, input_embed_size=D_m, output_embed_size=128, k=2)

        n_relations = 2 * n_speakers ** 2
        self.window_past = window_past
        self.window_future = window_future

        self.matchatt_pre = MatchingAttention(2 * D_e, 2 * D_e, att_type='general2')
        self.att_model = MaskedEdgeAttention(2 * D_e, max_seq_len, self.no_cuda)
        self.nodal_attention = nodal_attention
    ...
    def forward(self, data, att2=False):
        ...
        ### VGG-ResNet
        vggres_fea = self.vggres(U) # 3, batch, 128
        ...
        if self.base_model == "DialogRNN":
            ...
            lstm_input = torch.cat((U, vggres_fea), 2) # 3, batch, D_r
            emotions, hidden = self.lstm(lstm_input)
        ...
        features, edge_index, edge_norm, edge_type, edge_index_lengths = batch_graphify(emotions, qmask, lengths,
                                                                                        ...
                                                                                        self.edge_type_mapping,
                                                                                        self.att_model, self.no_cuda)
        log_prob = self.graph_net(features, edge_index, edge_norm, edge_type, lengths, umask, self.nodal_attention,
                                  self.avec)
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, downsample=False, downsample_type='resnet', upsample=True,
    ...
    def forward(self, x):
        ...
        if self.upsample:
            ...
            residual = residual.unsqueeze(dim=3)
            residual = F.upsample(Variable(residual), size=(E * 2, 1), mode='bilinear').squeeze(dim=3)
            residual = residual.transpose(1, 2)
...
class VDCNN(nn.Module):
    ...
    def __init__(self, depth, input_embed_size, output_embed_size, k):
        super(VDCNN, self).__init__()

        depth = depth  # 29
        input_embed_size = input_embed_size  # 100
        output_embed_size = output_embed_size  # 100
        optional_shortcut = True
        k = k  # 2
        ...
        if depth == 9:
            ...
            n_conv_layers = {'conv_block_512': 1, 'conv_block_256': 1,
                             'conv_block_128': 10, 'conv_block_64': 10}
        elif depth == 49:
            n_conv_layers = {'conv_block_512': 6, 'conv_block_256': 10,
                             'conv_block_128': 16, 'conv_block_64': 16}
        elif depth == 8:
            n_conv_layers = {'conv_block_400_2': 3, 'conv_block_200_2': 1, 'conv_block_100_2': 1,
                             'conv_block_400': 10, 'conv_block_200': 10, 'conv_block_100': 40}

        conv_layers = []
        conv_layers.append(nn.Conv1d(input_embed_size, 64, kernel_size=3, padding=1))

        for i in range(n_conv_layers['conv_block_64']):
            conv_layers.append(ResidualBlock(64, 64, upsample=False, optional_shortcut=optional_shortcut))

        for i in range(n_conv_layers['conv_block_128']):
            if i == 0:
                conv_layers.append(ResidualBlock(64, 128, optional_shortcut=optional_shortcut))
            conv_layers.append(ResidualBlock(128, 128, upsample=False, optional_shortcut=optional_shortcut))
    ...
    def forward(self, sentences):
        x = sentences
        x = x.transpose(0, 2)
        x = x.transpose(0, 1)  # (sequence_length, batch_size, embed_size) -> (batch_size, embed_size, sequence_length)
